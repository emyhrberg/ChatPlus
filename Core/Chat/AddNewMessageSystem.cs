...
internal class AddNewMessageSystem : ModSystem
{
                              ...
                              RemadeChatMonitor self, string text, Color color, int widthLimitInPixels)
    {
        try
        {
            ...
            if (Conf.C.timestampSettings != ClientConfig.TimestampSettings.Off)
            {
            ...
            if (!string.IsNullOrEmpty(senderName) && !string.IsNullOrEmpty(nameTagFull))
            {
                ...
                if (who >= 0)
                {
                    // Prefer ChatPlus synced color if set
                    if (PlayerColorSystem.PlayerColors.TryGetValue(who, out var syncedHex) &&
                        !string.IsNullOrWhiteSpace(syncedHex))
                    {
                        hex = syncedHex.ToUpperInvariant();

                        // If default color is set AND PvPAdventure team chat is enabled, use the team chat color.
                        bool isDefaultNameColor = Conf.C.PlayerColor == "FFFFFF";
                        if (isDefaultNameColor && ModLoader.TryGetMod("PvPAdventure", out _))
                        {
                            Player player = Main.player[who];
                            if (player.team > 0 && player.team < Main.teamColor.Length)
                            {
                                Color teamColor = Main.teamColor[player.team];
                                hex = $"{teamColor.R:X2}{teamColor.G:X2}{teamColor.B:X2}";
                            }
                        }
                    }
                }
        ...
        catch (Exception e)
        {
            ...
            Mod.Logger.Warn($"ModifyNewMessage failed; fell back to vanilla. {e}");
        }
    }

    private static string GetTimeFormat(ClientConfig.TimestampSettings setting)
    {
        if (setting == ClientConfig.TimestampSettings.HourAndMinute12Hours)
        {
            return "h:mm tt";
        }
        if (setting == ClientConfig.TimestampSettings.HourAndMinuteAndSeconds12Hours)
        {
            return "h:mm:ss tt";
        }
        if (setting == ClientConfig.TimestampSettings.HourAndMinute24Hours)
        {
            return "HH:mm";
        }
        if (setting == ClientConfig.TimestampSettings.HourAndMinuteAndSeconds24Hours)
        {
            return "HH:mm:ss";
        }
        return "HH:mm";
